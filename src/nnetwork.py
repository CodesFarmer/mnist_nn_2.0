...
class Network(object):
    ...
    def TrainNet(self,tr_d,va_d,epochs,lr,mode, relz="", parameters=""):
        ...
        """
        lmbda, rho, beta = translate_par(parameters)

        if(mode == "BGD"):
            self.BGD(tr_d,va_d,epochs,lr, relz,lmbda)
        elif(mode == "SGD"):
            self.SGD(tr_d,va_d,epochs,lr, relz,lmbda)
        elif(mode == 'ACSGD'):
    ...
    def SGD(self, tr_d, va_d, epochs, lr, relz="", lmbda=0.0):
        """
        ...
        while j < epochs:
            ...
            for tr_batch in batches:
                if(relz[0:2] != "DP"):
                    self.update_network(tr_batch,lr, relz, lmbda)
                else:
                    self.update_network_dp(tr_batch,lr,relz[2:], lmbda)
    ...
    def update_network(self, tr_d, lr, relz="", lmbda=0.0):
        ...
        if(relz == "L2"):
            self.weights = [w - (lr/trlen)*dw - (lr/trlen/100)*lmbda*w for w,dw in zip(self.weights, delta_w)]
        elif(relz == "L1"):
            self.weights = [w - (lr/trlen)*dw - (lr/trlen/100)*lmbda*np.sign(w) for w,dw in zip(self.weights, delta_w)]
        else:
    ...
    def backppg_ce(self,x,y):
        ...
        return (delta_b, delta_w)
    def plotimg(self,tr_d, epochs, lr, va_d=None):
        qcost_te = []
        z1 = []
        accuracy_te = []
        qcost_tr = []
        ...
        while j < epochs:
            ...
            acy, cost = self.Evaluate_a_c(va_d)
            qcost_te.append(cost)
            accuracy_te.append(acy)

            acy, cost = self.Evaluate_a_c(tr_d)
            qcost_tr.append(cost)
            accuracy_tr.append(acy)
            z1.append(i)

            i += 1

            if (va_d):
                print "Epoch {0}: {1}".format(j, self.Evaluate_a_c(va_d))
            else:
                print "Epoch {0}:".format(j)
            j += 1

        plotfunc.plotstatic(z1,qcost_te)
        plotfunc.plotstatic(z1, accuracy_te)
        plotfunc.plotstatic(z1,qcost_tr)
        plotfunc.plotstatic(z1, accuracy_tr)
